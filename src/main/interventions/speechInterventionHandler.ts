...
export default class SpeechInterventionHandler implements InterventionHandler {
  ...
  async recordAudio(maxDurationSeconds: number, autoDetect = true): Promise<string> {
    return new Promise((resolve) => {
      ...
      const argsManual = ['-d', '-c', '1', '-r', '16000', '-b', '16', '-e', 'signed-integer', outputPath, 'trim', '0', maxDurationSeconds.toString()];
      ...
      const argsAuto = ['-d', '-c', '1', '-r', '16000', '-b', '16', '-e', 'signed-integer', outputPath, 'silence', '1', '0.1', '3%', '1', '2.0', '3%'];
